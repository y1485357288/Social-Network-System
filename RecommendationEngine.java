...
public class RecommendationEngine {
    ...
    private String generateRecommendationReason(User targetUser, User user, int targetUserId, int userId) {
        ...
        // 显示目标用户的所有爱好
        if (targetUser.getInterests() != null && !targetUser.getInterests().isEmpty()) {
            reason.append("您的爱好：");
            for (int i = 0; i < targetUser.getInterests().size(); i++) {
                reason.append(targetUser.getInterests().get(i));
                if (i < targetUser.getInterests().size() - 1) {
                    reason.append("、");
                }
            }
            reason.append("；");
        ...
        if (targetUser.getInterests() != null && user.getInterests() != null) {
            ...
            if (!commonInterests.isEmpty()) {
                ...
                reason.append("（匹配数：").append(commonInterests.size()).append("）；");
            }
        }
